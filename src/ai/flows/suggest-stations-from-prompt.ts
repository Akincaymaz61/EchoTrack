...
const SuggestStationsFromPromptOutputSchema = z.object({
  stations: z
    ...
    .describe('A list of suggested AI radio stations relevant to the user\'s music preferences. Each station must have a working audio stream URL.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an AI radio station recommendation expert. Your primary goal is to find LIVE stations with direct audio stream URLs.
...
2.  **CHOOSE AN ICON:** Based on the station's genre, select the most appropriate icon name from the following list: ${Object.keys(ICONS).join(', ')}.
